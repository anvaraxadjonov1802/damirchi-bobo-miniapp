...
import { Bell, ChevronLeft, MapPin, Sparkles, Store } from "lucide-react";
...
export default function AppHeader({
  ...
  return (
    <header className="sticky top-0 z-40 safe-top border-b border-[var(--app-border-soft)] bg-[#FFFAF2] px-4 pb-3 pt-3 header-compositor">
      {isMenu ? (
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-1.5 flex min-w-0 items-center gap-1.5">
                ...
                <span className={`shrink-0 text-[10px] font-black uppercase tracking-[0.14em] ${isOpen ? "text-emerald-700" : "text-red-600"}`}>
                  {isOpen ? "Ochiq" : "Yopiq"}
                </span>
                ...
                <span className="truncate text-[11px] font-bold text-[var(--app-muted)]">{address}</span>
              </div>
            </div>
          </div>
          ...
          >
            ...
            {notificationCount > 0 && (
              <span className="absolute -right-1.5 -top-1.5 flex h-5 min-w-5 items-center justify-center rounded-full border-2 border-[#FFFAF2] bg-[#C89438] px-1 text-[10px] font-black text-white">
                {notificationCount}
              </span>
            )}
          </button>
        ...
        <div className="flex items-center justify-between gap-3">
          <button
            ...
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[18px] border border-[var(--app-border)] bg-white transition-click"
            aria-label="Orqaga"
          >
          ...
          <div className="min-w-0 flex-1 text-center">
            <h1 className="truncate text-[19px] font-black leading-tight tracking-[-0.035em] text-[var(--app-text)]">{title}</h1>
            <p className="mt-0.5 text-[10px] font-black uppercase tracking-[0.16em] text-[var(--app-muted)]">Damirchi</p>
          </div>
